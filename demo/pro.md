...
| GA/T 1078 TF卡（录像流）流播放，配套UI 倍率（2/4/8/16/32/64）选择                                                                                                                                                                                                                                    | 不支持 | 支持  |
...
| ws(s)/http(s)-fmp4 | 支持     | 支持     | 支持    | 支持   | 支持      | 支持      |
| ws(s)/http(s)-h264 | 支持     | 不支持    | 不支持   | 不支持   | 不支持     | 不支持     |
| ws(s)/http(s)-h265 | 不支持    | 支持     | 不支持   | 不支持   | 不支持     | 不支持     |
| http(s)-HLS-mp4    | 支持     | 支持     | 支持    | 不支持   | 不支持     | 不支持     |
| http(s)-HLS-ts     | 支持     | 支持     | 支持    | 支持    | 支持      | 支持      |
| http(s)-mpeg-ts    | 支持     | 支持     | 支持    | 支持    | 支持      | 支持      |
| http(s)-mpeg-ps    | 支持     | 支持     | 支持    | 支持    | 支持      | 支持      |
| WebTransport(M7S)       | 支持     | 支持     | 支持    | 支持    | 支持      | 支持      |
| Webrtc H264(M7S)        | 支持     | 不支持    | 不支持   | 不支持   | 支持      | 支持      |
| Webrtc H265(M7S)        | 支持     | 支持     | 支持    | 支持    | 支持      | 支持      |
| Webrtc H264(zlmediakit) | 支持     | 不支持    | 不支持   | 不支持   | 支持      | 支持      |
| Webrtc H264(SRS)        | 支持     | 不支持    | 不支持   | 不支持   | 支持      | 支持      |
| Webrtc H264(其他)         | 支持     | 不支持    | 不支持   | 不支持   | 支持      | 支持      |
...
> G711A、G711U音频编码格式 支持在 `http(s)-mpeg-ts` ，`http(s)-HLS-ts` 协议里面走自定义 stream_type
> 实现，如定制请咨询作者bosswancheng。
...
> G711A、G711U音频编码格式 支持在 `http(s)-HLS-ts` 协议里面走自定义 stream_type
> 实现，如定制请咨询作者bosswancheng。
